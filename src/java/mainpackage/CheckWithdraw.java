...
@WebServlet(name = "CheckWithdraw", urlPatterns = {"/CheckWithdraw"})
public class CheckWithdraw extends HttpServlet {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            ...
            if(val>0)
            {
                ...
                pstat2.setString(2,"Cheque Withdrawn with C No - " + num);
                pstat2.setDouble(3,amt);
                pstat2.setString(4,tdate);
                pstat2.executeUpdate();
                response.sendRedirect("checkwithdraw.jsp?msg=success");
            }
            else
            {
                response.sendRedirect("checkwithdraw.jsp?msg=error");
            }
