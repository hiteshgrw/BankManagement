...
public class CloseAccount extends HttpServlet {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            PreparedStatement pstat = conn.prepareStatement("update accountdet set status = 'Deactivated' where aid = " + aid);
            int val = pstat.executeUpdate();
            if(val>0)
            {
                response.sendRedirect("closeAccount.jsp?msg=success");
            }
            else
            {
                response.sendRedirect("closeAccount.jsp?msg=error");
            }
