...
@WebServlet(name = "Electricity", urlPatterns = {"/Electricity"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(val>0)
            {
                ...
                pstat2.setString(2,"Electricity Bill Payment of B/No - " + bnum + " A/No - " + anum + " " + opr + " using Netbanking");
                pstat2.setDouble(3,amt);
                pstat2.setString(4,tdate);
                pstat2.executeUpdate();
                response.sendRedirect("electricity.jsp?msg=success");
            }
            else
            {
                response.sendRedirect("electricity.jsp?msg=error");
            }
