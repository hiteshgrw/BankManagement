...
@WebServlet(name = "MobileRecharge", urlPatterns = {"/MobileRecharge"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(val>0)
            {
                ...
                pstat2.setString(2,"Mobile Recharge of " + mnum + " " + opr + " using Netbanking");
                pstat2.setDouble(3,amt);
                pstat2.setString(4,tdate);
                pstat2.executeUpdate();
                response.sendRedirect("mobileRecharge.jsp?msg=success");
            }
            else
            {
                response.sendRedirect("mobileRecharge.jsp?msg=error");
            }
